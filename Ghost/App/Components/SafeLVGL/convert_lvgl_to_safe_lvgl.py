...
regex = r"^(?P<return_type>\w{1}.+)[ ]{1}(?P<function_name>(lv_){1}.+)(\((?P<var_list>[a-zA-Z.*,_ ]+)\);)"

#TODOl: FIX `lv_label_set_text_fmt(lv_obj_t * obj, const char * fmt, ...) LV_FORMAT_ATTRIBUTE(2, 3);` !!

blacklist_declare_keys = ["typedef"]
blacklist_function_name_keys = [
    "static"
    ]
...
blacklist_files = []
blacklist_file_keys = [
    "nxp", "sdl", "dma2d",  # Please use manual locking for special platform library.
    "printf", "log",        # Com
    "tlsf", "sw", "lv_lru",
    "ffmpeg", "freetype", "rlottie",   # freetype, ffmpeg is useless in mcu.
    "gridnav", "lv_monkey"
    ]
...
def find_lv_function_declare(code : str):
    ...
    """
    for key in blacklist_declare_keys:
        if(key in code):
            return None
    ...
    if(ret != None):
        ...
        for key in blacklist_function_name_keys:
...
def main(lvgl_path : str, safe_lvgl_path : str):
    ...
    for parent, dirnames, filenames in os.walk(lvgl_path):
        ...
        is_white_path = False
        while(os.path.relpath(parent_path) != os.path.relpath(lvgl_path)):
            for path in whitelist_paths:
                if(os.path.relpath(parent_path) == os.path.relpath(os.path.join(lvgl_path, path))):
                    is_white_path = True
                    break
            ...
            for filename in filenames:
                ...
                if(is_legal_file):
                    ...
                    if ext =="h":
                        ...
                        output_header_file.write("#include \"lvgl.h\"\n\n")
